...
const DodgeGame = () => {
  ...
  const LOGIC_HEIGHT = 750;
  
  // Game constants
  const PLAYER_SIZE = 12;
  const GRAVITY_STRENGTH = 0.032;
  ...
  const createDot = () => {
    ...
    if (side === 0) { // Top
      ...
      x = LOGIC_WIDTH + buffer;
      y = Math.random() * LOGIC_HEIGHT;
    } else if (side === 2) { // Bottom
      x = Math.random() * LOGIC_WIDTH;
      y = LOGIC_HEIGHT + buffer;
    } else { // Left
      x = -buffer;
      y = Math.random() * LOGIC_HEIGHT;
    }
    ...
    const targetX = LOGIC_WIDTH / 2 + (Math.random() - 0.5) * 400;
    const targetY = LOGIC_HEIGHT / 2 + (Math.random() - 0.5) * 400;
  ...
  const gameLoop = () => {
    ...
    if (player.x > LOGIC_WIDTH - PLAYER_SIZE) {
      player.x = LOGIC_WIDTH - PLAYER_SIZE;
      player.vx = 0;
    }
    ...
    if (player.y > LOGIC_HEIGHT - PLAYER_SIZE) {
      player.y = LOGIC_HEIGHT - PLAYER_SIZE;
      player.vy = 0;
    }
    ...
    for (let i = dots.length - 1; i >= 0; i--) {
      ...
      const buffer = 150;
      if (dot.x < -buffer || dot.x > LOGIC_WIDTH + buffer || dot.y < -buffer || dot.y > LOGIC_HEIGHT + buffer) {
        dots[i] = createDot();
      }
    }
    ...
    dots.forEach(dot => {
      ...
      if (dist < GRAVITY_RADIUS) {
        const opacity = ((GRAVITY_RADIUS - dist) / GRAVITY_RADIUS) * 0.4;
        ctx.beginPath();
  ...
  return (
    <div className="not-prose relative w-full aspect-[16/10] bg-slate-900 rounded-[2rem] overflow-hidden shadow-2xl border border-white/5 font-sans mb-12">
      <canvas 
        ref={canvasRef} 
        width={LOGIC_WIDTH} 
        height={LOGIC_HEIGHT} 
      ...
      <div className="absolute top-10 left-12 pointer-events-none">
        <div className="text-white font-mono text-3xl font-black tracking-widest drop-shadow-lg opacity-60">
          {score.toFixed(1)}s
        </div>
      </div>
      ...
      {gameState === 'START' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/60 backdrop-blur-sm transition-all z-20">
          <div className="bg-slate-800/90 p-12 rounded-[3rem] border border-white/10 shadow-2xl flex flex-col items-center max-w-md mx-4">
            <h2 className="text-6xl font-black text-white mb-2 tracking-tighter italic">DODGE</h2>
            <div className="w-16 h-1 bg-indigo-500 mb-8"></div>
            <p className="text-slate-300 text-lg mb-10 font-light text-center leading-relaxed">
              Your ship attracts nearby dots.<br/>
              Use <span className="text-indigo-400 font-mono font-bold">WASD</span> or <span className="text-indigo-400 font-mono font-bold">ARROWS</span> to move.
            </p>
            <button 
              onClick={startGame}
              className="group relative px-12 py-5 bg-indigo-600 hover:bg-indigo-500 text-white text-xl font-bold rounded-full transition-all shadow-xl shadow-indigo-500/25 active:scale-95"
            >
              START [SPACE]
      ...
      {gameState === 'GAMEOVER' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-red-950/40 backdrop-blur-md transition-all z-20">
          <div className="bg-slate-900/95 p-12 rounded-[3rem] border border-red-500/30 shadow-2xl flex flex-col items-center min-w-[380px] mx-4">
            <h2 className="text-6xl font-black text-white mb-2 tracking-tighter italic">CRASHED</h2>
            <div className="text-3xl text-red-400 font-bold mb-10">
              TIME: {score.toFixed(1)}s
            </div>
            
            {highScores.length > 0 && (
              <div className="mb-10 w-full bg-black/40 rounded-[2rem] p-8 border border-white/5">
                <h3 className="text-xs font-mono font-bold text-slate-500 uppercase tracking-[0.3em] mb-6 text-center">Top Survivals</h3>
                <div className="space-y-3">
                  {highScores.map((hs, i) => (
                    <div key={i} className="flex justify-between text-white font-mono text-lg">
            ...
            <button 
              onClick={startGame}
              className="w-full py-5 bg-white text-slate-900 text-xl font-black rounded-full transition-all hover:scale-[1.02] active:scale-95 shadow-xl"
            >
              RETRY [SPACE]
            </button>
          </div>
